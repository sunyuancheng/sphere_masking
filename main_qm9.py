...
import os
from logger import FileLogger, WandbLogger, TBLogger
...
def main(args):
    ...
    tb_logger = TBLogger(is_master=is_main_process, is_rank0=is_main_process, output_dir=args.output_dir)
    _log.info(args)
    ...
    for epoch in range(args.epochs):
        ...
        train_err = train_one_epoch(model=model, criterion=criterion, norm_factor=norm_factor,
            ...
            print_freq=args.print_freq, logger=_log, tb_logger=tb_logger)
        ...
        _log.info(info_str)
        tb_logger.log(update_dict={"mae": val_err},
                      step=epoch,
                      split="valid")
        tb_logger.log(update_dict={"mae", test_err},
                      step=epoch,
                      split="test")
        ...
        if model_ema is not None:
            ...
            info_str += 'Time: {:.2f}s'.format(time.perf_counter() - epoch_start_time)
            _log.info(info_str)
            tb_logger.log(update_dict={"mae": ema_val_err},
                        step=epoch,
                        split="valid_ema")
            tb_logger.log(update_dict={"mae", ema_test_err},
                        step=epoch,
                        split="test_ema")
